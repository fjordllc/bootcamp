...
class Products::UnassignedProductsComponentTest < ViewComponent::TestCase
  ...
  def test_default
    products = [products(:product1), products(:product2), products(:product3), products(:product4), products(:product5), products(:product6),
                products(:product7), products(:product8), products(:product9)]
    products_grouped_by_elapsed_days = group_by_elapsed_days(products)
    render_inline(Products::UnassignedProductsComponent.new(products:,
  ...
  def test_any_products_elapsed_reply_warning_days
    ...
    render_inline(Products::UnassignedProductsComponent.new(products: products_within_reply_warning_days,
                                                            products_grouped_by_elapsed_days:,
                                                            is_mentor: @is_mentor,
                                                            current_user_id: @current_user.id,
                                                            reply_warning_days: @reply_warning_days))
  ...
  def any_products_almost_passed_reply_warning_days
    ...
    render_inline(Products::UnassignedProductsComponent.new(products: products_passed_reply_warning_days,
                                                            products_grouped_by_elapsed_days:,
                                                            is_mentor: @is_mentor,
                                                            current_user_id: @current_user.id,
                                                            reply_warning_days: @reply_warning_days))
