...
class PracticeTest < ActiveSupport::TestCase
  ...
  test '#status(user)' do
    assert_equal \
      PracticeLearnerRecord.new(practices(:practice1)).status(users(:komagata)),
      'started'

    assert_equal \
      PracticeLearnerRecord.new(practices(:practice1)).status(users(:machida)),
      'unstarted'
  end
  ...
  test '#status_by_learnings(learnings)' do
    ...
    assert_equal PracticeLearnerRecord.new(practices(:practice1)).status_by_learnings(learnings), 'started'
    assert_equal PracticeLearnerRecord.new(practices(:practice2)).status_by_learnings(learnings), 'complete'
    assert_equal PracticeLearnerRecord.new(practices(:practice3)).status_by_learnings(learnings), 'unstarted'
    assert_equal PracticeLearnerRecord.new(practices(:practice4)).status_by_learnings(learnings), 'submitted'
    assert_equal PracticeLearnerRecord.new(practices(:practice5)).status_by_learnings(learnings), 'unstarted'
  end

  test '#exists_learning?(user)' do
    assert PracticeLearnerRecord.new(practices(:practice1)).exists_learning?(users(:komagata))
    assert_not PracticeLearnerRecord.new(practices(:practice1)).exists_learning?(users(:machida))
  end
  ...
  test '#category' do
    ...
    assert_equal category, PracticeCategoryResolver.new(practice).category(course)
  end
