...
module Transcoder
  class ClientTest < ActiveSupport::TestCase
    ...
    def client(force_video_only: false)
      Transcoder::Client.new(@valid_movie, config: @valid_config, bucket_name: "bucket", project_id: "proj", force_video_only: force_video_only)
    end
    ...
    test "location required in config" do
      ...
      config["location"] = nil
      error = assert_raises(ArgumentError) { Transcoder::Client.new(@valid_movie, config: config, bucket_name: "bucket", project_id: "proj") }
    ...
    test "#transcode creates job" do
      c = client
      job_created = false
      with_transcoder_service(c, list_jobs: [], create_job_proc: ->(args) { job_created = true })
      c.transcode
    ...
    test "#transcode skips if job exists" do
      ...
      with_transcoder_service(c, list_jobs: [OpenStruct.new(labels: { "movie_id" => @valid_movie.id.to_s }, state: "RUNNING")],
                               create_job_proc: ->(args) { job_created = true })
      c.transcode
      refute job_created
    end
    ...
    test "#transcode raises on failure" do
      c = client
      with_transcoder_service(c, list_jobs: [], create_job_proc: ->(args) { raise Google::Cloud::Error, "API failed" })
    ...
    test "#transcode force_video_only" do
      ...
      refute_includes mux_keys, "audio-stream"
    end

    test "#get_movie_id returns id" do
      c = client
      with_transcoder_service(c, get_job_proc: ->(args) { OpenStruct.new(labels: { 'movie_id' => '123' }) })
    ...
    test "#get_movie_id nil on error" do
      c = client
      with_transcoder_service(c, get_job_proc: ->(args) { raise Google::Cloud::Error, "boom" })
