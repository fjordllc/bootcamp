...
module EventSchedule
  class RegularEventScheduleTest < ActiveSupport::TestCase
    ...
    test 'specified week event #tentative_next_event_date' do
      travel_to Time.zone.local(2023, 12, 31, 0, 0, 0) do
        schedule = EventSchedule::RegularEventSchedule.new(@specified_week_event)
        assert_equal Time.zone.local(2024, 1, 1, 16, 0, 0), schedule.tentative_next_event_date
      end

      travel_to Time.zone.local(2024, 1, 2, 0, 0, 0) do
        schedule = EventSchedule::RegularEventSchedule.new(@specified_week_event)
        assert_equal Time.zone.local(2024, 2, 5, 16, 0, 0), schedule.tentative_next_event_date
      end
    end
    ...
    test 'specified week event #held_next_event_date' do
      travel_to Time.zone.local(2023, 12, 31, 0, 0, 0) do
        schedule = EventSchedule::RegularEventSchedule.new(@specified_week_event)
        assert_equal Time.zone.local(2024, 2, 5, 16, 0, 0), schedule.held_next_event_date
      end
    end
  end
end
