...
class QuestionTest < ActiveSupport::TestCase
  ...
  test '#clear_question_cache_skips_when_question_is_wip' do
    questioner = users(:kimura)
    answerer = users(:komagata)
    question = Question.create!(
      title: 'テストの質問',
    ...
    Cache.stub(:delete_not_solved_question_count, -> { flunk 'Should not be called' }) do
      logs = []
      Rails.logger.stub(:info, ->(msg) { logs << msg }) do
        question.clear_question_cache
        assert_empty logs
      end
    end
  end

  test '#clear_question_cache_skips_when_question_is_solved' do
    questioner = users(:kimura)
    answerer = users(:komagata)
    question = Question.create!(
      title: 'テストの質問',
  ...
  test '#clear_question_cache_clears_cache_and_logs_when_question_is_unsolved_and_published' do
    questioner = users(:kimura)
    answerer = users(:komagata)
    question = Question.create!(
      title: 'テストの質問',
    ...
    called = false
    logs = []
    Cache.stub(:delete_not_solved_question_count, -> { called = true }) do
      Rails.logger.stub(:info, ->(msg) { logs << msg }) do
        question.clear_question_cache
      end
    end
    ...
    assert called
    assert_includes logs, '[before_destroy] Cache destroyed for unsolved question count.'
  end
