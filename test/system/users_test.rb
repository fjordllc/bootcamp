...
class UsersTest < ApplicationSystemTestCase
  ...
  test 'access by other users' do
    user = users(:hatsuno)
    visit_with_auth edit_admin_user_path(user.id), 'mentor'
    assert_text '管理者としてログインしてください'
  end

  test 'graduation date is displayed' do
    visit_with_auth "/users/#{users(:mentor).id}", 'komagata'
    assert_no_text '卒業日'
  ...
  test 'show users role' do
    ...
    visit_with_auth "/users/#{users(:mentor).id}", 'mentor'
    assert_text 'メンター'
