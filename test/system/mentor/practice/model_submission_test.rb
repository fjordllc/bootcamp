...
class Mentor::Practices::ModelSubmissionTest < ApplicationSystemTestCase
  test 'mentor can create model answer' do
    practice = practices(:practice2)
    practice.model_submission = nil

    visit_with_auth new_mentor_practice_model_submission_path(practice), 'komagata'
    fill_in '内容', with: '模範解答内容です。'
    click_on '登録する'
    assert_current_path practice_model_submission_path(practice)
  ...
  test 'mentor can update model answer' do
    ...
    visit_with_auth edit_mentor_practice_model_submission_path(practice), 'komagata'
