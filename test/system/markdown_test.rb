...
class MarkdownTest < ApplicationSystemTestCase
  ...
  test 'should expand link card even when nested within details markdown syntax' do
    visit_with_auth new_report_path, 'komagata'
    within('form[name=report]') do
      fill_in('report[title]', with: 'details内でもリンクカードが展開される')
      fill_in(
        'report[description]',
        with: <<~TEXT
          :::details detailsではネストしていもリンクカードが展開されるはず
    ...
    within('details') do
      assert_selector '.a-link-card', visible: false
      assert_no_selector 'a.before-replacement-link-card[href="https://bootcamp.fjord.jp/"]', visible: false
    end
  end

  test 'should expand link card even when nested within message markdown syntax' do
    visit_with_auth new_report_path, 'komagata'
    within('form[name=report]') do
      fill_in('report[title]', with: 'message内でもリンクカードが展開される')
      fill_in(
        'report[description]',
        with: <<~TEXT
          :::message
