...
class API::Admin::FaqTest < ApplicationSystemTestCase
  test 'sort FAQ' do
    visit faq_path
    assert_equal all('h3.lp-faq__title').first.text, '学習を終えるのにどれくらいの時間がかかりますか？'
    assert_equal all('h3.lp-faq__title').last.text, 'フィヨルドブートキャンプを利用していることを誰にも知られたくないのですが、匿名で参加はできますか？'
  ...
  test "doesn't affect the order of other FAQs" do
    visit faq_path
    assert_equal all('h3.lp-faq__title')[0].text, '学習を終えるのにどれくらいの時間がかかりますか？'
    assert_equal all('h3.lp-faq__title')[1].text, '途中で辞めることは可能ですか？'
