...
module Products
  class LearningStatusTest < ApplicationSystemTestCase
    test 'should change learning status when change wip status' do
      ...
      ProductChecker.new(product).change_learning_status(:started)
      ...
      ProductChecker.new(other_product).change_learning_status(:started)

      ProductChecker.new(product).change_learning_status(:submitted)
      visit product_path
