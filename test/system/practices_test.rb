...
class PracticesTest < ApplicationSystemTestCase
  ...
  test 'show link to all practices with same category' do
    ...
    category = PracticeCategoryResolver.new(practice1).category(user.course)
  ...
  test 'category button link to courses/practices#index with category fragment' do
    ...
    user = users(:komagata)
    category = PracticeCategoryResolver.new(practice).category(user.course)
  ...
  test 'change status' do
    ...
    assert_equal 'started', PracticeLearnerRecord.new(practice).status(users(:hatsuno))
  end
