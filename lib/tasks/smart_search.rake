...
namespace :smart_search do
  ...
  task :generate_embeddings, [:model_name] => :environment do |_task, args|
    ...
    unless BulkEmbeddingJob::SEARCHABLE_MODELS.include?(model_name)
      puts "Invalid model name: #{model_name}"
      puts "Available models: #{BulkEmbeddingJob::SEARCHABLE_MODELS.join(', ')}"
      exit 1
    end
  ...
  task stats: :environment do
    ...
    BulkEmbeddingJob::SEARCHABLE_MODELS.each do |model_name|
