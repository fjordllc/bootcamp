...
class UserUnstartedPracticesQuery < Patterns::Query
  ...
  def query
    relation
      ...
      .select('practices.*, courses_categories.position AS category_position, categories_practices.position AS practice_position')
      .order('practice_position ASC, category_position ASC')
  end
