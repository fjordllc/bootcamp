...
class ActivityMailer < ApplicationMailer # rubocop:todo Metrics/ClassLength
  ...
  def came_answer(args = {})
    ...
    message.perform_deliveries = @user.mail_notification? && !UserStatus.new(@user).retired?
  ...
  def cancel_pair_work(args = {})
    ...
    message.perform_deliveries = @receiver.mail_notification? && !UserStatus.new(@receiver).retired?
    message
  end
end
