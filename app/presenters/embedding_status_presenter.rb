...
class EmbeddingStatusPresenter
  ...
  def build_statuses(model_statuses)
    ...
    model_statuses_array = model_statuses.except('overall').map do |model_name, status_hash|
      Status.new(model_name, status_hash)
    end

    # 合計を最後に追加
    overall = Status.new('合計', model_statuses['overall'], total_row: true)
    model_statuses_array << overall
  end

  class Status
    attr_reader :name, :total, :with_embedding, :without_embedding, :percentage, :total_row, :last_updated
