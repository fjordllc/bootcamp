document.addEventListener('DOMContentLoaded', () => {
  const reactions = document.querySelectorAll('.js-answer-reactions')
  ...
  const createAnswerReaction = (reaction, kind, loginName, reactionableId) => {
    const answerUrl = `/api/reactions?reactionable_answer_id=${reactionableId}&kind=${kind}`

    requestReaction(answerUrl, 'POST', (json) => {
  ...
  const destroyAnswerReaction = (reaction, kind, loginName, reactionId) => {
    const answerUrl = `/api/reactions/${reactionId}`

    requestReaction(answerUrl, 'DELETE', (json) => {
      Array.from(reaction.querySelectorAll(`[data-reaction-kind="${kind}"]`), element => {
  ...
  Array.from(reactions, reaction => {
    ...
    Array.from(reaction.querySelectorAll('li'), element => {
      element.addEventListener('click', e => {
        ...
        if (reactionId) {
          destroyAnswerReaction(reaction, kind, loginName, reactionId)
        } else {
          createAnswerReaction(reaction, kind, loginName, reactionableId)
        }
      })
    })
  })
