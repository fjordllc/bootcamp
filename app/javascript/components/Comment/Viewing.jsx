...
const Description = ({ comment }) => {
  const markdownDescription = new MarkdownInitializer().render(
    comment.description
  )
...
const CommentViewing = ({
  ...
  const commentURL =
    window.location.href.split('#')[0] + '#comment_' + comment.id
    ...
    <Card.Root>
      <Card.Header>
        ...
        <h2 className="thread-comment__title">
          <a
            className="thread-comment__title-user-link is-hidden-md-up"
            href={comment.user.url}>
            <img
              className={clsx(
                'thread-comment__title-user-icon a-user-icon',
                roleClass
              )}
          ...
          <a
            className="thread-comment__title-link a-text-link"
            href={comment.user.url}>
