...
json.student UserStatus.new(user).student?
json.card UserBilling.new(user).card?
json.job_name t("activerecord.enums.user.job.#{user.job}")
json.os_name t("activerecord.enums.user.os.#{user.os}")
json.student_or_trainee UserStatus.new(user).student_or_trainee?
...
if UserStatus.new(user).hibernated?
  json.hibernated_at l(user.hibernated_at, format: :year_and_date)
  json.hibernation_elapsed_days UserHibernation.new(user).hibernation_elapsed_days
end
