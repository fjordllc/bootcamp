...
module SmartSearch
  class EmbeddingGenerator
    ...
    BATCH_SIZE = 40
    MAX_TOKENS = 20_000 # Google AI APIの制限

    def initialize
      @project_id = Rails.application.credentials.dig(:google_cloud, :project_id) || ENV['GOOGLE_CLOUD_PROJECT']
    ...
    def calculate_optimal_batch_size(texts, max_tokens = MAX_TOKENS)
    ...
    def generate_embedding(text)
      ...
      begin
        ...
        raise e if Rails.env.development? # 開発環境では詳細エラーを表示
    ...
    def generate_embeddings_batch(texts)
      ...
      begin
        ...
        Rails.logger.error "Failed to generate batch embeddings: #{e.message}"
        []
      end
    end
    ...
    def parse_embedding_response(response)
      # レスポンス構造をデバッグ出力
      Rails.logger.debug "Response structure: #{response.predictions.first}" if Rails.env.development?
    ...
    def endpoint_path
      ...
      "projects/#{project_id}/locations/#{location}/publishers/google/models/#{EMBEDDING_MODEL}"
    end
  end
end
