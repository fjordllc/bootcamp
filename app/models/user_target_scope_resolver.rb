...
# 画面から渡されるtarget/job文字列を、渡されたRelationのscopeへ安全に解決する責務を持つ。
class UserTargetScopeResolver
  ...
  # このメソッドはユーザから送信された値をpublic_sendに渡すので、悪意のあるコードが実行される危険性がある
  ...
  # users_roleと同じく安全性確保のため、以下の条件を指定している。
