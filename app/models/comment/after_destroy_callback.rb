...
class Comment::AfterDestroyCallback
  def after_destroy(comment)
    ...
    ProductCommentedAtTracking.new(comment.commentable).delete_last_commented_at
    ProductCommentedAtTracking.new(comment.commentable).delete_commented_at
  ...
  def delete_assigned_and_unreplied_product_count_cache(comment)
    ...
    return unless product.checker_id.present? && ProductCommentedAtTracking.new(product).replied_status_changed?(comment.previous&.user_id, comment.user_id)
