...
class AiPrompt < ApplicationRecord
  DEFINITIONS = {
    'article_meta_description' => {
      ...
      path: 'article_meta_description_agent/instructions.txt.erb'
    },
    'pjord' => {
      name: 'ピヨルド共通',
      description: 'ピヨルドがすべての応答で共通して守る役割や振る舞いを定めます。',
      path: 'pjord/agent/instructions.txt.erb'
    },
    'mention_response' => {
      name: 'メンション返信',
      description: 'ユーザーからピヨルドへのメンションに返信する方針を定めます。',
      path: 'pjord/mention_response_agent/instructions.txt.erb'
    },
    'product_review' => {
      name: '提出物レビュー',
      description: '提出物をレビューし、コメントを作成する方針を定めます。',
      path: 'pjord/product_review_agent/instructions.txt.erb'
    },
    'question_answer' => {
      name: 'Q&A回答',
      description: 'Q&Aに投稿された質問へ回答する方針を定めます。',
      path: 'pjord/question_answer_agent/instructions.txt.erb'
    },
    'report_classifier' => {
      name: '日報分類',
      description: '日報の内容から、コメントの意図を分類する方針を定めます。',
      path: 'pjord/report_classifier_agent/instructions.txt.erb'
    },
    'report_comment' => {
      name: '日報コメント',
      description: '分類された意図に沿って日報へのコメントを作成する方針を定めます。',
      path: 'pjord/report_comment_agent/instructions.txt.erb'
    }
  }.freeze
  ...
  class << self
    ...
    def default_body_for(key)
      return Pjord::Agent::SYSTEM_PROMPT if key.to_s == 'pjord'
