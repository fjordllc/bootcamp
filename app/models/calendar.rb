...
class Calendar
  ...
  def special_calendar_str(user)
    special_calendar = SpecialEventsCalendar.new
    special_calendar_to_ical = special_calendar.convert_to_ical(user).to_ical
    special_calendar_to_ical.gsub(/END:VCALENDAR\r?\n?\z/, '')
  end

  def regular_calendar_str(user)
    regular_calendar = RegularEventsCalendar.new
    regular_calendar_to_ical = regular_calendar.convert_to_ical(user).to_ical
