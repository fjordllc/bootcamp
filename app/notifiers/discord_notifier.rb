...
class DiscordNotifier < ApplicationNotifier # rubocop:disable Metrics/ClassLength
  ...
  def hibernated(params = {})
    params.merge!(@params)
    # Safely read webhook URL with string/symbol fallback
    config = Rails.application.config_for(:secrets)
    webhook_url = params[:webhook_url] ||
                  config.dig('webhook', 'admin') ||
                  config.dig(:webhook, :admin)
  ...
  def first_report(params = {})
    ...
    config = Rails.application.config_for(:secrets)
    webhook_url = params[:webhook_url] ||
                  config.dig('webhook', 'introduction') ||
                  config.dig(:webhook, :introduction)
