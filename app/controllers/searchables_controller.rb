...
class SearchablesController < ApplicationController
  ...
  def index
    ...
    searchables = Searcher.search(word: @word, document_type: @document_type, current_user:)
    @searchables = Kaminari.paginate_array(searchables).page(params[:page]).per(PER_PAGE)
