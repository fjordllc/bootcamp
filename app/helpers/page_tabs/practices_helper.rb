...
module PageTabs
  module PracticesHelper
    def practice_page_tabs(practice, active_tab:, include_source: nil)
      ...
      append_conditional_tabs(tabs, practice)
      render PageTabsComponent.new(tabs:, active_tab:)
    end

    private

    def append_conditional_tabs(tabs, practice)
      ...
      tabs << { name: '理解度テスト', link: practice_practice_quiz_path(practice) } if practice.quiz_gate.practice_quiz_required?
      tabs << { name: '理解度テスト管理', link: mentor_practice_quiz_path(practice) } if admin_or_mentor_login?
