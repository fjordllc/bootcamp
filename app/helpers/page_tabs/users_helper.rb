...
module PageTabs
  module UsersHelper
    def user_page_tabs(user, active_tab:)
      ...
      tabs << { name: '日報', link: user_reports_path(user), count: user.reports.length, badge: unchecked_report_badge }
      ...
      tabs << { name: 'イベント', link: user_events_path(user), count: user.participate_events.length + user.participate_regular_events.length }
